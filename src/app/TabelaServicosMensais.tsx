...
const TabelaServicosMensais: FC<{
  ...
  const [escalaMensal, setEscalaMensal] = useState<ServicoMensal>(
    gerarServicoMensal({ mes, ano }),
  );
  const [permutas, setPermutas] = useState<
    Array<{ dia: number; de: string; para: string }>
  >([]);
  ...
  function handleOnClickCelula({
    ...
    if (isEditingPermuta) {
      setEscalaMensal((prev) => {
        ...
        const servicoASerTrocadoIndex = servicosDoDia.findIndex(
          (s) => s.dono === permuta.matricula,
        );
          ...
          <thead>
            <tr>
              {Array.from({ length: escalaMensal.length }, (_, i) => (
                ...
                >
                  {i + 1} -{" "}
                  {new Date(ano, mes - 1, i + 1).toLocaleDateString("pt-BR", {
                    weekday: "short",
                  })}
                </th>
              ))}
              ...
              {escalaMensal.map((servicoDia, index) => (
                ...
                >
                  {servicoDia.map((servico, idx) => {
                    const isDispensado = dispensas?.some(
                      (d) =>
                        d.matricula === servico.dono && d.dia === index + 1,
                    );
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor:
                            isEditingPermuta &&
                            permuta.matricula === servico.dono
                              ? "#ffeb3b"
                              : isAllowedPermuta
                                ? "#e0f7fa"
                                : "#a6a5a5",
                        ...
                        onClick={() =>
                          handleOnClickCelula({
                            diaIndex: index,
                            matricula: servico.dono,
                          })
                        }
                      >
                        {servico.permuta ? (
                          <>
                            {efetivo.find((s) => s.id === servico.permuta)
                              ?.name || servico.permuta}{" "}
                            ({servico.permuta})
                            <br />
                            <span
                              style={{
                            ...
                            >
                              {" "}
                              (permuta de{" "}
                              {efetivo.find((s) => s.id === servico.dono)
                                ?.name || servico.dono}{" "}
                              ({servico.dono}))
                            </span>
                          </>
                          ...
                          <>
                            {efetivo.find((s) => s.id === servico.dono)?.name ||
                              servico.dono}{" "}
                            ({servico.dono}){" "}
                            {efetivo.find((s) => s.id === servico.dono)
                              ?.isAdventist
      ...
      <div style={{ marginTop: "1rem", padding: "1rem 2rem" }}>
        ...
        <ul>
          {permutas.map((p, index) => (
            <li key={index}>
              {efetivo.find((s) => s.id === p.de)?.name || p.de} ({p.de})
              permuta com {efetivo.find((s) => s.id === p.para)?.name || p.para}{" "}
              ({p.para}) no dia {p.dia + 1}
            </li>
          ))}
        </ul>
      </div>
